...
class M_admin extends CI_Model
{
    ...
    function get_all_admin($offset=0,$limit=0,$order_by="firstname",$type="asc",$filter = array()) {
        $g_list = array();
    ...
    function get_admin_by_perm($perm) {
        $g_list = array();
        $g_list2 = array();
        $this->db->select('admin_user.id, admin_user.firstname, admin_user.lastname, admin_user.email,');
        $this->db->where('perm.permission', $perm);
        $this->db->where('admin_user.receive_mail', "y");
        $this->db->join('admin_user', 'admin_user.id = perm.admin_id');
        $query = $this->db->get('perm');
