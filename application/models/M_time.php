...
class M_time extends CI_Model {
  ...
  public function __construct(){
	...
	function datepicker_to_unix_excel ($data) {
		@$str=explode("/", $data);
		if (@strlen($str[2])==2) {
			@$str[2]="20".$str[2];
		}
		return @mktime(0,0,1,(int)$str[0],(int)$str[1],((int)$str[2]-543));
	}
	function datepicker_to_unix_ad ($data) {
		@$str=explode("/", $data);
	...
	function unix_to_datetimepicker_reverse ($data) {
		$true_date="";
		if ($data>1000) {
			$date_string=date("Y/m/d H:i",$data);
			@$str1=explode(" ", $date_string);
			@$str2=explode("/", $str1[0]);
			@$str3=explode(":", $str1[1]);
			@$true_date=$str2[2]."/".$str2[1]."/".((int)$str2[0]+543)." ".$str1[1];
		}
		return $true_date;
	}
